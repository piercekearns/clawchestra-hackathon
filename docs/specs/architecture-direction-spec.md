...
### Future scope: cross-platform apps and "Login with OpenClaw"

The architecture we're building (data endpoint, client identity, session keys, programmatic sync) is designed to support mobile and web clients in the future. The end state: a user downloads Clawchestra on any platform, connects to their OpenClaw, and all their projects are there.

"Login with OpenClaw" is a potential standard — instead of Login with Google/GitHub, users connect to their own OpenClaw instance. OpenClaw carries identity, project state, and AI access across every platform. This is future scope, not part of the current architecture direction work.

See roadmap item `clawchestra-apps` for tracking mobile/web/cross-platform work.
...
What GitHub login would provide is already handled by other mechanisms:
- Pre-authenticated git operations → `gh auth` / SSH keys
- User identity → OpenClaw connection
- Repository discovery → scan paths + git remote detection

What it would cost: OAuth flow, GitHub API dependency, privacy reduction (app talks to GitHub's servers).

Git CLI with existing auth handles code operations. If GitHub login becomes useful later (e.g., for a web app), it can be revisited as part of `clawchestra-apps`.
